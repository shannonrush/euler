...
Lychrel <- function() {
    sum(sapply(1:9999, IsLychrel))
}

IsLychrel <- function(n) {
    # return 1 if n doesn't produce a palindrome within 50 iterations, otherwise return 0
    result <- n
    for (i in 1:49) {
        digits <- unlist(strsplit(toString(result),""))
        rev.digits <- rev(digits)
        result <- result+as.numeric(paste(rev.digits,collapse=""))
        if (IsPalindrome(result)) return(0)
    }
    1
}
...
# Lychrel() 2.409992 2.409992 2.409992 2.409992 2.409992 2.409992     1