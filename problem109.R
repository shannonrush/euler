...
Darts <- function() {
    singles <- c(1:20,25)
    doubles <- sapply(singles, prod, 2)
    trebles <- sapply(1:20, prod, 3)
    throws <- sort(c(singles,doubles,trebles))
    ways <- 0
    for (i in doubles) {
        if (i<100) ways <- ways + 1
    }

    for (i in throws) {
        for (j in doubles) {
            if (i+j<100) ways <- ways + 1  
        }
    }
    ...
    for (i in 1:length(throws)) {
        for (j in 1:i) {
            for (k in doubles) {
                if (throws[i]+throws[j]+k < 100) ways <- ways + 1
            }   
        }
    }
    ways
...
# Darts() 44.14349 45.12533 52.44735 46.68396 49.52087 102.3382    10
